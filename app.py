...
with st.sidebar:
    ...
    st.caption("All sources are public and free.")
...
if page == "Overview":
    ...
    with col1:
        ...
        if not df_prices.empty:
            for _, row in df_prices.iterrows():
                ...
                st.markdown(
                    f"**{row['Commodity']}** &nbsp; {row['Price']:.2f} {row['Unit']} "
                    f"&nbsp; {icon} {row['Change %']:+.2f}% &nbsp; *({row['Date']})*"
                )
        else:
            st.warning("Could not load prices. Please check your connection.")

    with col2:
        st.subheader("BRL/USD Rate")
    ...
    if not df_exp.empty:
        ...
        st.info("Export data currently unavailable.")


# ── PAGE 2: INTERNATIONAL PRICES ────────────────────────────────────
elif page == "International Prices":
    st.title("International Prices")
    st.markdown("Monthly historical series. Source: World Bank Commodity Price Data (Pink Sheet).")
    st.divider()
    ...
    if not df.empty:
        fig = px.line(
            df, x="date", y="price",
            title=f"{commodity} — International price ({months} months)",
        ...
        col_a.metric("Current price", f"{df['price'].iloc[-1]:.2f}")
        col_b.metric("High",          f"{df['price'].max():.2f}")
        col_c.metric("Low",           f"{df['price'].min():.2f}")
    else:
...
# ── PAGE 3: BRAZIL EXPORTS ──────────────────────────────────────────
    ...
    with st.spinner(f"Loading {commodity} export data ({year})..."):
        df_exp = get_exports_by_commodity(commodity, year=year)

    if not df_exp.empty:
        st.subheader(f"Top destinations — {commodity} ({year})")
...
# ── PAGE 4: BRL VS COMMODITIES ──────────────────────────────────────
    ...
    if not df_fx.empty and not df_com.empty:
        ...
        fig.update_layout(
            ...
            yaxis2=dict(title="Price", titlefont=dict(color="#C9A547"),
        ...
        if len(df_merged) >= 3:
            ...
            st.metric("Correlation BRL/USD x Price", f"{corr:.2f}",
                      help="1 = perfect positive correlation, -1 = inverse")
    else:
        st.warning("Insufficient data for this chart.")

# ── FOOTER ──────────────────────────────────────────────────────────
st.divider()
st.caption("Commodity Price Monitor / Brazil · hugotolomei.com · Data: World Bank, MDIC, BCB")
